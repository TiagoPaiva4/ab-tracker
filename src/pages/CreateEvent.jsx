  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedParticipants.length > 0) {
        ...
        if (attError) console.error("Erro participantes:", attError);
      }
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            <label className="input-field" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: '#6b7280' }}>
              <Upload size={20} />
              {file ? file.name : "Clique para carregar foto"}
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              <button 
                type="button" 
                onClick={toggleAll}
                style={{ fontSize: '0.8rem', color: 'var(--color-primary)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}
              >
                {selectedParticipants.length === GROUP_MEMBERS.length ? 'Desmarcar Todos' : 'Selecionar Todos'}
              </button>
            </div>
